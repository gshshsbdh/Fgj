...
.tb-right{display:flex;align-items:center;gap:8px;flex-wrap:wrap}
 .tunnel-flow-card{margin-bottom:16px}.tunnel-steps{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-top:12px}.tunnel-step{background:var(--bg3);border:1px solid var(--card-b);border-radius:14px;padding:14px;display:flex;flex-direction:column;gap:5px}.tunnel-step span{width:28px;height:28px;border-radius:8px;background:var(--accent-d);color:var(--accent2);display:flex;align-items:center;justify-content:center;font-weight:800}.tunnel-step b{font-size:12px}.tunnel-step small{font-size:10px;color:var(--t3);line-height:1.7}@media(max-width:900px){.tunnel-steps{grid-template-columns:1fr 1fr}}@media(max-width:600px){.tunnel-steps{grid-template-columns:1fr}}
    ...
    <div class="cm-body">
      ...
      <div class="cm-section">
        ...
        <div class="cm-field"><label>نام کانفیگ</label>
          <input class="cm-input" id="nl-label" placeholder="هش خودکار ۷ حرفی یا نام دلخواه">
        </div>
...
<!-- مودال نصب Agent Rathole -->
<div class="modal-bg" id="modal-rathole-install" style="z-index:10000">
  <div class="modal-v2" style="max-width:680px">
    <button class="modal-v2-close" onclick="closeModal('modal-rathole-install')"><i class="ti ti-x"></i></button>
    <div class="modal-v2-head">
      <div class="modal-v2-icon" style="background:linear-gradient(135deg,var(--accent),var(--accent2))"><i class="ti ti-terminal-2"></i></div>
      <div class="modal-v2-title">اتصال نود ایران</div>
      <div class="modal-v2-sub" id="rathole-install-sub">این فرمان را فقط روی همان سرور ایران اجرا کن</div>
    </div>
    <div class="modal-v2-body">
      <div class="cl amber" style="margin-top:0"><i class="ti ti-alert-triangle"></i><span>اجرای این دستور، فقط Agent و Rathole Client را روی سرور ایران نصب می‌کند. پنل اصلی روی Railway باقی می‌ماند.</span></div>
      <div style="margin-top:14px">
        <label>فرمان نصب</label>
        <textarea class="cm-input mono" id="rathole-install-command" rows="5" readonly style="direction:ltr;text-align:left;resize:vertical"></textarea>
      </div>
      <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:10px;flex-wrap:wrap">
        <button class="btn btn-g" onclick="selectRatholeInstallCommand()"><i class="ti ti-select-all"></i> انتخاب کل</button>
        <button class="btn btn-p" onclick="copyRatholeInstallCommand()"><i class="ti ti-copy"></i> کپی فرمان</button>
      </div>
      <div class="cm-note" style="margin-top:12px"><i class="ti ti-info-circle"></i><span>بعد از اجرای فرمان، ۵ تا ۱۵ ثانیه صبر کن و رفرش بزن. وضعیت باید از «آفلاین» به «آنلاین» تغییر کند.</span></div>
    </div>
  </div>
</div>
<!-- مودال بروزرسانی -->
...
<aside class="sidebar" id="sb">
  ...
  <div class="nav-wrap">
    ...
    <div class="nav-it" data-pg="rathole"><i class="ti ti-route"></i> تانلینگ <span class="nav-badge" id="rathole-nb">0</span></div>
...
<section class="pg" id="pg-links">
  <div class="topbar">
    <div style="display:flex;justify-content:flex-end;gap:8px;margin-bottom:16px">
      <button class="btn btn-p" onclick="prepareCreateLink();openModal('modal-create-link')">
...
<section class="pg" id="pg-rathole">
  <div class="topbar">
    <div><div class="tb-title"><i class="ti ti-route"></i> تانلینگ</div>
    <div class="tb-sub">ساخت کانفیگ، انتشار پورت، اتصال دامنه و تست مسیر در یک جریان ساده</div></div>
    <div class="tb-right"><span class="badge bg-blue">Rathole TCP</span><button class="btn btn-p btn-sm" onclick="loadRathole()"><i class="ti ti-refresh"></i> رفرش</button></div>
  </div>

  <div class="card tunnel-flow-card">
    <div class="card-title"><i class="ti ti-route-2"></i> مسیر پیشنهادی</div>
  ...
  <div class="g2">
    <div class="card">
      <div class="card-title"><i class="ti ti-server-2"></i> نودهای ایران
        <button class="btn btn-p btn-sm" style="margin-right:auto" onclick="createRatholeNode()"><i class="ti ti-plus"></i> نود جدید</button>
      </div>
      <div id="rathole-nodes" class="list-wrap"><div class="muted">در حال بارگذاری...</div></div>
    </div>
    <div class="card">
      <div class="card-title"><i class="ti ti-settings"></i> تنظیمات Rathole</div>
      <div class="g2" style="margin:0">
        <div><label>پورت کنترل</label><input class="cm-input" id="rh-server-port" type="number" min="1" max="65535" value="23333"></div>
        <div><label>پورت پیش‌فرض</label><input class="cm-input" id="rh-public-port" type="number" min="0" max="65535" value="443"><div class="cm-note" style="margin-top:6px"><i class="ti ti-info-circle"></i><span>اگر پورت استفاده شده باشد، پورت آزاد بعدی خودکار انتخاب می‌شود.</span></div></div>
      </div>
  ...
  <div class="card" style="margin-top:16px">
    ...
    <div class="g2" style="margin:0">
      <div>
        <label>کانفیگ آماده</label>
        <select class="cm-input" id="tun-link-select" onchange="applyTunnelPresetFromLink()">
          <option value="">انتخاب کانفیگ...</option>
        </select>
        <div class="cm-note" style="margin-top:8px"><i class="ti ti-info-circle"></i><span id="tun-link-info">برای شروع یک کانفیگ از همین پنل انتخاب کن.</span></div>
      </div>
      <div>
        <label>نود ایران</label>
        <select class="cm-input" id="tun-node-select">
          <option value="">انتخاب نود...</option>
        </select>
        <label style="margin-top:10px">پورت محلی سرویس</label>
        <input class="cm-input" id="tun-local-port" type="number" min="1" max="65535" value="443">
      </div>
    </div>
    <div style="display:flex;justify-content:flex-end;margin-top:12px">
      <button class="btn btn-p" onclick="createTunnelFromWorkflow()"><i class="ti ti-player-play"></i> انتشار پورت و ساخت تانل</button>
    </div>
  </div>

  <div class="card" style="margin-top:16px">
    <div class="card-title"><i class="ti ti-arrows-left-right"></i> تانل‌های منتشرشده</div>
    <div id="rathole-tunnels" style="display:flex;flex-direction:column;gap:12px"><div class="muted">در حال بارگذاری...</div></div>
  </div>

  <div class="card" style="margin-top:16px">
    <div class="card-title"><i class="ti ti-shield-check"></i> محدوده‌های Cloudflare</div>
    <div class="cm-note" style="margin-bottom:10px"><i class="ti ti-info-circle"></i><span>این CIDRها قابل تنظیم‌اند و فقط policy پنل را نگه می‌دارند؛ خودشان firewall Railway را تغییر نمی‌دهند.</span></div>
    <div class="g2" style="margin:0">
      <textarea class="cm-input mono" id="rh-cf-v4" rows="4" placeholder="IPv4 CIDR، هر خط یک مورد"></textarea>
      <textarea class="cm-input mono" id="rh-cf-v6" rows="4" placeholder="IPv6 CIDR، هر خط یک مورد"></textarea>
    </div>
    <div style="display:flex;justify-content:flex-end;margin-top:10px"><button class="btn btn-p" onclick="saveCloudflareRanges()"><i class="ti ti-device-floppy"></i> ذخیره محدوده‌ها</button></div>
  </div>
</section>
...
function navTo(name){
  ...
  const loaders={links:loadLinks,connections:loadConns,errors:loadErrs,subscriptions:loadSubsPage,subgroups:loadSubs,logs:loadActivity,rathole:loadRathole,updates:loadVersion,support:loadSupportMsgs,nodes:loadNodesPage};  if(loaders[name])loaders[name]();
...
function makeDefaultConfigLabel(){
...
function prepareCreateLink(){
  const input=document.getElementById('nl-label');
  if(input&&!input.value.trim()) input.value=makeDefaultConfigLabel();
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setInterval(() => {
    ...
    if (document.getElementById('pg-rathole').classList.contains('on')) loadRathole();
...
async function loadTunnelLinks(){
  try{
    const r=await authF('/api/links'); const d=await r.json();
    tunnelLinksCache=d.links||[];
    const sel=document.getElementById('tun-link-select');
    if(!sel)return;
    sel.innerHTML='<option value="">انتخاب کانفیگ...</option>'+tunnelLinksCache.map(x=>{
      const proto=(x.protocol||'').toUpperCase();
      return `<option value="${rEsc(x.uuid)}">${rEsc(x.label||x.uuid)} · ${rEsc(proto)}</option>`;
    }).join('');
  }catch(e){}
}
function renderTunnelNodeSelect(){
  const sel=document.getElementById('tun-node-select'); if(!sel)return;
  sel.innerHTML='<option value="">انتخاب نود...</option>'+ratholeCache.nodes.map(n=>`<option value="${rEsc(n.node_id)}" ${n.online?'':'disabled'}>${rEsc(n.label)} ${n.online?'· آنلاین':'· آفلاین'}</option>`).join('');
}
function applyTunnelPresetFromLink(){
  const uid=document.getElementById('tun-link-select')?.value;
  const info=document.getElementById('tun-link-info');
  const x=tunnelLinksCache.find(v=>v.uuid===uid);
  if(!info)return;
  if(!x){info.textContent='برای شروع یک کانفیگ از همین پنل انتخاب کن.';return;}
  const proto=String(x.protocol||'').toLowerCase();
  const suggested=proto.includes('mtproto')?(x.mtproto_port||443):443;
  document.getElementById('tun-local-port').value=suggested;
  info.textContent=`${x.label||x.uuid} · ${x.protocol||'—'} · پورت پیشنهادی ${suggested}`;
}
async function createTunnelFromWorkflow(){
  const nodeId=document.getElementById('tun-node-select').value;
  const uid=document.getElementById('tun-link-select').value;
  const port=Number(document.getElementById('tun-local-port').value||443);
  if(!nodeId||!uid){toast('کانفیگ و نود ایران را انتخاب کن','warn');return;}
  const link=tunnelLinksCache.find(v=>v.uuid===uid);
  const protocol=(link?.protocol||'vless-ws').toLowerCase();
  const isWs=protocol.includes('ws') || protocol.includes('websocket');
  try{
    const r=await authF('/api/rathole/tunnels',{method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({node_id:nodeId,name:link?.label||'Tunnel '+uid.slice(0,6),local_host:'127.0.0.1',local_port:port,public_port:Number(document.getElementById('rh-public-port').value||443),protocol:isWs?'websocket':'tcp',websocket_path:'/',nodelay:true})});
    const d=await r.json(); if(!r.ok)throw new Error(d.detail||'خطا');
    toast('پورت منتشر شد؛ حالا دامنه خارج تونل را وارد کن','ok');
    loadRathole();
    setTimeout(()=>focusTunnelBinding(d.tunnel?.id),250);
  }catch(e){toast(e.message||'خطا','err')}
}
function focusTunnelBinding(id){
  const el=document.getElementById('tunnel-bind-'+id);
  if(el){el.scrollIntoView({behavior:'smooth',block:'center'});el.focus?.();}
}
function rEsc(v){return String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]));}
async function loadRathole(){
  try{
    const r=await authF('/api/rathole/status'); if(!r.ok) throw new Error();
    const d=await r.json(); ratholeCache=d;
    document.getElementById('rathole-nb').textContent=d.nodes.length;
    document.getElementById('rh-server-port').value=d.settings.server_bind_port;
    ...
    renderRatholeNodes(); renderTunnelNodeSelect(); renderRatholeTunnels(); loadTunnelLinks();
  }catch(e){toast('خطا در دریافت وضعیت Rathole','err')}
}
function renderRatholeNodes(){
  const el=document.getElementById('rathole-nodes');
  if(!ratholeCache.nodes.length){
    el.innerHTML='<div class="muted">هنوز نودی ثبت نشده.</div>';
    return;
  }
  el.innerHTML=ratholeCache.nodes.map(n=>{
    const offline=!n.online;
    return `<div class="sr" style="padding:12px 0">
      <span class="sr-k">
        <i class="ti ti-server"></i>
        <b>${rEsc(n.label)}</b>
        <small style="display:block;color:var(--dim);margin-top:3px">
          ${rEsc(n.hostname||'—')} · ${rEsc(n.platform||'—')}
        </small>
      </span>
      <span class="sr-v" style="display:flex;align-items:center;gap:7px;flex-wrap:wrap">
        <span class="badge ${n.online?'bg-green':'bg-red'}">
          ${n.online?'● آنلاین':'● منتظر اتصال'}
        </span>
        ${offline ? `<button class="btn btn-p btn-sm" onclick="showRatholeInstall('${rEsc(n.node_id)}','${rEsc(n.label)}')">
          <i class="ti ti-plug-connected"></i> اتصال Node
        </button>` : `<span class="badge bg-green">Rathole Ready</span>`}
        <button class="btn btn-g btn-sm" onclick="showRatholeInstall('${rEsc(n.node_id)}','${rEsc(n.label)}')" title="فرمان نصب Agent">
          <i class="ti ti-terminal-2"></i>
        </button>
        <button class="btn btn-g btn-sm" onclick="deleteRatholeNode('${rEsc(n.node_id)}')" title="حذف Node">
...
function renderRatholeTunnels(){
 const el=document.getElementById('rathole-tunnels');
 if(!ratholeCache.tunnels.length){el.innerHTML='<div class="muted">هنوز پورت منتشر نشده.</div>';return;}
 el.innerHTML=ratholeCache.tunnels.map(t=>{
   ...
   const extPath=t.external_path||'/';
   return `<div class="card" id="tunnel-row-${rEsc(t.id)}" style="padding:15px;margin:0">
     <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap">
       <div>
         <div style="font-weight:800;font-size:13px">${rEsc(t.name)}</div>
     ...
     <div class="g2" style="margin:14px 0 0">
       ...
       <div class="cl" style="margin:0"><i class="ti ti-brand-cloudflare"></i><span>دامنه بیرونی: <b>${extHost? rEsc(extHost+':'+extPort):'تنظیم نشده'}</b></span></div>
     </div>
     <div class="g2" style="margin:12px 0 0" id="tunnel-bind-${rEsc(t.id)}">
       <div><label>دامنه/آی‌پی خارج تونل</label><input class="cm-input" id="bind-host-${rEsc(t.id)}" value="${rEsc(extHost)}" placeholder="example.com"></div>
       <div><label>پورت</label><input class="cm-input" id="bind-port-${rEsc(t.id)}" type="number" value="${extPort}" min="1" max="65535"></div>
     </div>
     <div class="g2" style="margin:10px 0 0">
       <div><label>نوع endpoint</label>
         <select class="cm-input" id="bind-scheme-${rEsc(t.id)}">
           ${['tcp','https','http','wss','ws'].map(v=>`<option value="${v}" ${v===extScheme?'selected':''}>${v.toUpperCase()}</option>`).join('')}
         </select>
       </div>
       <div><label>Path (برای HTTP/WebSocket)</label><input class="cm-input" id="bind-path-${rEsc(t.id)}" value="${rEsc(extPath)}" placeholder="/"></div>
     </div>
     <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:12px;align-items:center">
       <span class="badge bg-blue">${t.last_ping_ms!=null?`Ping ${t.last_ping_ms} ms`:'Ping نشده'}</span>
       <button class="btn btn-g btn-sm" onclick="bindTunnelExternal('${rEsc(t.id)}')"><i class="ti ti-plug-connected"></i> اتصال دامنه</button>
       <button class="btn btn-p btn-sm" onclick="pingTunnel('${rEsc(t.id)}')"><i class="ti ti-activity"></i> Ping</button>
     </div>
   </div>`;
 }).join('');
}
async function bindTunnelExternal(id){
  const host=document.getElementById('bind-host-'+id)?.value.trim();
  const port=Number(document.getElementById('bind-port-'+id)?.value||0);
  const scheme=document.getElementById('bind-scheme-'+id)?.value||'tcp';
  const path=document.getElementById('bind-path-'+id)?.value||'/';
  if(!host||!port){toast('دامنه و پورت را وارد کن','warn');return}
  try{
    const r=await authF('/api/rathole/tunnels/'+encodeURIComponent(id)+'/bind',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({host,port,scheme,path})});
    const d=await r.json();if(!r.ok)throw new Error(d.detail||'خطا');
    toast('دامنه خارج تونل ثبت شد','ok');loadRathole();
  }catch(e){toast(e.message||'خطا','err')}
}
...
async function createRatholeNode(){
 const label=prompt('نام نود ایران:','Iran Node'); if(label===null)return;
 try{
   const r=await authF('/api/rathole/nodes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({label})});
   const d=await r.json();
   if(!r.ok)throw new Error(d.detail||'خطا');
   await loadRathole();
   await showRatholeInstall(d.node_id,d.label,d.install_command);
 }catch(e){toast(e.message||'خطا','err')}
}
async function showRatholeInstall(nodeId,label,existingCommand){
 try{
   let command=existingCommand;
   if(!command){
     const r=await authF('/api/rathole/nodes/'+encodeURIComponent(nodeId)+'/install-command',{method:'POST'});
     const d=await r.json();
     if(!r.ok)throw new Error(d.detail||'خطا در صدور فرمان نصب');
     command=d.install_command;
   }
   document.getElementById('rathole-install-sub').textContent='نود «'+(label||'Iran Node')+'» · فرمان جدید و یک‌بارمصرف';
   document.getElementById('rathole-install-command').value=command;
   openModal('modal-rathole-install');
 }catch(e){toast(e.message||'خطا','err')}
}
function selectRatholeInstallCommand(){
 const el=document.getElementById('rathole-install-command'); el.focus(); el.select();
 try{el.setSelectionRange(0,el.value.length)}catch(e){}
}
async function copyRatholeInstallCommand(){
 const el=document.getElementById('rathole-install-command');
 const text=el.value||''; if(!text)return;
 try{
   if(navigator.clipboard && window.isSecureContext){await navigator.clipboard.writeText(text);toast('فرمان کپی شد ✓','ok');return;}
   throw new Error('clipboard-unavailable');
 }catch(e){
   selectRatholeInstallCommand();
   try{document.execCommand('copy');toast('فرمان کپی شد ✓','ok');}
   catch(_){toast('فرمان انتخاب شد؛ گزینه کپی مرورگر را بزن','warn')}
 }
}
async function deleteRatholeNode(id){if(!confirm('این نود و تمام تونل‌هایش حذف شود؟'))return;await authF('/api/rathole/nodes/'+encodeURIComponent(id),{method:'DELETE'});loadRathole()}
async function openRatholeTunnelModal(){
 const online=ratholeCache.nodes.filter(n=>n.online);
 if(!online.length){toast('ابتدا یک نود آنلاین اضافه کن','warn');return;}
 const node=online[0];
 const name=prompt('نام تونل:','HTTPS 443'); if(name===null)return;
 const localPort=Number(prompt('پورت محلی روی سرور ایران:','443')); if(!localPort)return;
 const publicPort=Number(prompt('پورت عمومی روی Railway:','443')); if(!publicPort)return;
 ...
 try{
   ...
   const d=await r.json();
   if(!r.ok)throw new Error(d.detail||'خطا');
   toast(protocol==='websocket'?'تونل WebSocket ساخته شد':'تونل TCP ساخته شد','ok');
   loadRathole();
 }catch(e){toast(e.message||'خطا','err')}
}
...
async function saveRatholeSettings(){
 const body={server_bind_port:Number(document.getElementById('rh-server-port').value),public_base_port:Number(document.getElementById('rh-public-port').value),nodelay:document.getElementById('rh-nodelay').checked,transport:document.getElementById('rh-transport')?.value||'tcp'};
 const r=await authF('/api/rathole/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok){toast(d.detail||'خطا','err');return}toast('تنظیمات ذخیره شد','ok');
}
async function saveCloudflareRanges(){
 const v4=document.getElementById('rh-cf-v4').value.split(/\n|,/).map(x=>x.trim()).filter(Boolean);const v6=document.getElementById('rh-cf-v6').value.split(/\n|,/).map(x=>x.trim()).filter(Boolean);
 const r=await authF('/api/rathole/settings',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({cloudflare_ipv4:v4,cloudflare_ipv6:v6})});const d=await r.json();if(!r.ok){toast(d.detail||'خطا','err');return}toast('محدوده‌ها ذخیره شد','ok');
}
...
<div class="wrap">
  <div class="top">
    <div class="brand">
      <div class="brand-img"><i class="ti ti-shield-check" aria-hidden="true"></i></div>
      <div><div class="brand-name">Subscription</div><div class="brand-sub">Private access profile</div></div>
    </div>
    <div class="top-actions">
      <button class="icon-btn" id="theme-toggle" onclick="toggleTheme()" title="تغییر تم"><i class="ti ti-sun" id="theme-icon"></i></button>
      <a class="icon-btn" href="/sub-group/{uuid_key}" title="لینک سابسکریپشن"><i class="ti ti-link"></i></a>
    </div>
  </div>
  <div id="root">
    <div class="empty-state"><i class="ti ti-loader-2" style="animation:spin 1s linear infinite"></i>در حال بارگذاری...</div>
  </div>
  <div class="footer">لینک سابسکریپشن: <a href="/sub-group/{uuid_key}">/sub-group/{uuid_key}</a></div>
</div>
<script>
